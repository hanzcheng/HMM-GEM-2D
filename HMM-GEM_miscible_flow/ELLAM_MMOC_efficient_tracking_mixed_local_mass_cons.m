...
%   
%   HMM-GEM scheme for the miscible flow model on generic meshes
%
...
tic;
%% choose whether ELLAM or GEM
pureELLAM = 0; %default choice:GEM
% pureELLAM = 1; %ELLAM
...
%isHexa = 1;
% mesh={'thinverylowmesh_2_3.mat'};
% mesh={'slightly_Kershaw_2_3_4.mat'};
% mesh = {'thin2.mat'};
loadmesh=strcat('load ../../../matlab_meshes/',mesh{1}); % should change this to directory containing the meshes
eval(loadmesh);

%scale the mesh since original mesh was for domain (0,1)x(0,1)
diam=diam*1000;
area=area*1000^2;
vertex=vertex*1000;
center=center*1000;
...
%% time step
if isHexa==1
    tstep = 18; %need smaller timestep for hexa in order to achieve local volume conservation
else
    tstep=36; 
end
...
xprod=0;
yprod=0;    % x and y coordinates of the production point(s)
xinj=1000;
yinj=1000;  % x and y coordinates of the injection point(s)
...
%that are located along triangle n of the submesh e.g. sub_cell_ep(1,:)
%gives the points which are involved with triangle 1
sub_cell_ep=whichPoint(ncell,sub_ncell,cell_e,nxEdges,nyEdges,pointsToTrack,MpPerEdge+2);

%% determine initially which triangles are involved with a given point
% for example, triInvI(1,:) will give the triangles involved with the point
% pointsToTrack(1,:)
...
%% Classify regions and points as MMOC or ELLAM regions by approximating which cells are near injection
allCells=1:ncell;
[nearInjCell,nearProdCell,nearProdCellOnly,TriNearInj,TriNearProd,pNearInj,pNearProd]=classifyPtsandRegions(ncell,center,diam,mnbe,cell_e,sub_cell_ep,subCellsIn,icell,pcell);
...
for m=1:nbiter
    ...
    for i=1:ncell
        ...
        JAc(posc)=i;
        if startGEM>0
            VAc(posc)=alfa*area(i)+0.5*qm(i)+0.5*qp(i); %ELLAM MMOC mix
        else
    ...
    if startGEM>0
        [newPointsB,newPointsF,nextTriB,nextTriF]=complete_tracking_efficient(sub_cell_e,sub_cell_n,sub_cell_v,sub_vertex,sub_N,sub_nvert,sub_triInvVert,sIntercept,tstep,cornerPoints,pointsToTrack,pNearInj,pNearProd,triInvI,KRDarcyU);
    else
        [newPointsB,newPointsF,nextTriB,nextTriF]=complete_tracking_efficient(sub_cell_e,sub_cell_n,sub_cell_v,sub_vertex,sub_N,sub_nvert,sub_triInvVert,sIntercept,tstep,cornerPoints,pointsToTrack,pNearInj,pNearProdIni,triInvI,KRDarcyU);
    end
    %% compute area of intersection of the polygons
    if startGEM>0
        [AreaInB,AreaTb]=compAreaF(ncell,nearProdCell,subCellsIn,nxEdges,pointsToTrack, newPointsB,sub_cell_ep,pNearProd);
    else
    ...
    %% postprocess the tracked regions for local mass conservation 
    %comment out for tests without local mass conservation
    if startGEM>0
       [AreaInB,pctErrB,aErrB,nAdjB,pctErrBini] = adjust_volumes_for_local_mass(AreaInF,AreaInB,alffa,invTF,area,ncell,cell_e,cell_n,newPointsB,nextTriB,triInvI,subCellsIn,sub_cell_ep,mainCell,pOnEdge,icell,pcell,KRDarcyU,nearProdCellOnly,nearInjCell,startGEM,pNearProd,1);
        [AreaInF,pctErrF,aErrF,nAdjF,pctErrFini] = adjust_volumes_for_local_mass(AreaInF,AreaInF,alffa,invTF,area,ncell,cell_e,cell_n,newPointsF,nextTriF,triInvI,subCellsIn,sub_cell_ep,mainCell,pOnEdge,icell,pcell,-KRDarcyU,nearInjCell,nearProdCellOnly,startGEM,pNearInj,0);
    else
        if isHexa==0 %only do post-processing for ELLAM if not hexa mesh, since hexa mesh cannot yield converging adjustments
       [AreaInB,pctErrB,aErrB,nAdj,pctErrIni] = adjust_volumes_for_local_mass(AreaInF,AreaInB,alffa,invTF,area,ncell,cell_e,cell_n,newPointsB,nextTriB,triInvI,subCellsIn,sub_cell_ep,mainCell,pOnEdge,icell,pcell,KRDarcyU,allCells,0,startGEM,pNearProdIni,1);
        end
    end
    %%
    intCb=zeros(1,ncell);
    intCf=zeros(1,ncell);
    if startGEM>0 %remove the extra regions perturbed due to local mass balance adjustment
        AreaInB(nearInjCell,:)=0;
            AreaInF(:,nearProdCellOnly)=0;
    end
    for i=1:ncell
        intCb(i)=AreaInB(:,i)'*cPrev(1:ncell); %integral of c, backtracked
        intCf(i)=AreaInF(i,:)*cPrev(1:ncell); %integral of c, forward tracked
        if startGEM>0 %ELLAM-MMOC
            bc(i) = bc(i) -0.5*qm(i)*cPrev(i) + intCb(i)*Phi/tstep + intCf(i)*Phi/tstep-0.5*cPrev(i)*qp(i)+qp(i);
        else %pure ELLAM
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % SOLUTION for concentration equation
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    c=Ac\bc;
