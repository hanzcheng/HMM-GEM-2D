...
function [W,G] = local_flux_matrix(vertex,area,cg,L);

% vertex = (nb vertices + 1)x 2 array of vertex coordinates
%		vertex(i,:) = coordinates of the i-th vertex of the cell K
%			The first vertex is repeated at the end of the matrix, so that
%			vertex(i,:)-vertex(i+1,:), for i=1,nb vertices, represents the
%			edges of the cell.
...
% midpoints of edges
xs=zeros(nbe,2);
xs([1:nbe],:)=(vertex([1:nbe],:)+vertex([2:nbe+1],:))/2;

% Outer normals: we start by rotating by -pi/2 the vectors vertex(i+1,:)-vertex(i,:)
...
N=zeros(nbe,2);
N([1:nbe],:)=(vertex([2:nbe+1],:)-vertex([1:nbe],:))*[0 -1;1 0];
...
% Matrix D_K=diag(|sigma|/d_Ksigma).
%		d_Ksigma=(xs-xk).n_Ksigma.
%D=trace(L)*diag(msigma./diag(X*N'));
